...
func TestLink_GetList_Empty(t *testing.T) {
	ts := NewTestServer(t)

	w := ts.DoRequest("GET", "/api/v1/links")
	if w.Code != http.StatusOK {
		t.Errorf("期望状态码 200, 实际 %d", w.Code)
	}
}

func TestLink_GetList_WithData(t *testing.T) {
	ts := NewTestServer(t)

	ts.DB.Create(&entity.Link{Name: "示例网站", URL: "https://example.com", Status: "approved"})
...
func TestLink_Create_Unauthorized(t *testing.T) {
	ts := NewTestServer(t)

	body := `{"name":"新友链","url":"https://example.com"}`
	w := ts.DoRequestWithBody("POST", "/api/v1/admin/links", body)
...
func TestLink_Create_Success(t *testing.T) {
	ts := NewTestServer(t)
	ts.Login(t, "admin", "123456")

	body := `{"name":"新友链","url":"https://example.com","description":"一个示例网站"}`
	w := ts.DoAuthRequestWithBody("POST", "/api/v1/admin/links", body)
...
func TestLink_Update_Success(t *testing.T) {
	...
	l := entity.Link{Name: "原始友链", URL: "https://original.com", Status: "approved"}
	ts.DB.Create(&l)

	body := `{"name":"更新后的友链","url":"https://updated.com"}`
	w := ts.DoAuthRequestWithBody("PUT", "/api/v1/admin/links/"+uintToStr(l.ID), body)
...
func TestLink_UpdateStatus(t *testing.T) {
	ts := NewTestServer(t)
	ts.Login(t, "admin", "123456")

	l := entity.Link{Name: "待审核友链", URL: "https://pending.com", Status: "pending"}
	ts.DB.Create(&l)

	body := `{"status":"approved"}`
	w := ts.DoAuthRequestWithBody("PUT", "/api/v1/admin/links/"+uintToStr(l.ID)+"/status", body)
...
func TestLink_Delete_Success(t *testing.T) {
	ts := NewTestServer(t)
	ts.Login(t, "admin", "123456")

	l := entity.Link{Name: "待删除友链", URL: "https://delete.com", Status: "approved"}
	ts.DB.Create(&l)

	w := ts.DoAuthRequest("DELETE", "/api/v1/admin/links/"+uintToStr(l.ID))
	if w.Code != http.StatusOK {
		t.Errorf("期望状态码 200, 实际 %d, body=%s", w.Code, w.Body.String())
	}
}

func TestLink_GetAdminList(t *testing.T) {
	ts := NewTestServer(t)
	ts.Login(t, "admin", "123456")

	ts.DB.Create(&entity.Link{Name: "管理友链", URL: "https://admin.com", Status: "approved"})

	w := ts.DoAuthRequest("GET", "/api/v1/admin/links?page=1&page_size=10")
	if w.Code != http.StatusOK {
		t.Errorf("期望状态码 200, 实际 %d", w.Code)
	}
}
